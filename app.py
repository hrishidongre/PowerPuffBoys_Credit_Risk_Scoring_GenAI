"""
app.py  —  Primary Entrypoint  —  Agentic Credit Risk Assistant
...
""",
    ...
    with ftab1:
        ...
        with c2:
            ...
            tot_tl = st.number_input(
                "Total Trade Lines", 
                key="tot_tl",
                value=safe_int(prefill.get("Total_TL", 5), 5), min_value=0
            )
        with c3:
    ...
    with ftab3:
        ...
        with c1:
            age = st.number_input(
                "Age", 
                key="age_input",
                value=safe_int(prefill.get("AGE", 30), 30), min_value=18, max_value=80
            )
        ...
        with c2:
            gender = st.selectbox("Gender", ["M", "F"], key="gender_input")
            marital_status = st.selectbox(
                "Marital Status", ["Single", "Married", "Divorced"], key="marital_input"
            )
        ...
        with c3:
            last_prod = st.selectbox(
                "Last Product Enquiry", ["PL", "CC", "AL", "HL", "others"], key="last_prod_input"
            )
            first_prod = st.selectbox(
                "First Product Enquiry", ["PL", "CC", "AL", "HL", "others"], key="first_prod_input"
            )
    ...
    if analyze_clicked:
        try:
            with st.spinner(
                "Running agent  —  Risk Analysis  >  Guideline Retrieval  >  Report Generation..."
            ):
